...
/**
 * Verify KoboToolbox API token validity
 */
...
/**
 * Convert FieldScore questionnaire to XLSForm format
 */
export function questionnaireToXLSForm(questionnaire: Questionnaire): string {
  ...
  const surveyRows = (questionnaire.questions || [])
    .map((q, idx) => {
      ...
      return [type, name, label, '', relevant, required, '']
        .join('\t')
        .trim();
    })
    .join('\n');
    ...
    (questionnaire.questions || []).forEach((q) => {
      if ((q.type === 'select_one' || q.type === 'select_multiple') && q.options) {
        ...
        q.options.forEach((opt) => {
          choicesRows.push(
            [listName, opt.value, opt.label].join('\t')
          );
        });
      }
    });
...
/**
 * Map FieldScore question types to XLSForm types
 */
...
/**
 * Create or update an asset on KoboToolbox
 */
export async function createKoboAsset(
  ...
  try {
    ...
    // Convert XLSForm string to file
...
/**
 * Deploy an asset to make it active
 */
export async function deployKoboAsset(
  token: string,
  assetUid: string
): Promise<KoboDeployment> {
  try {
    const response = await fetch(
      `${KOBO_BASE_URL}/assets/${assetUid}/deployment/`,
      {
        method: 'POST',
        headers: {
          Authorization: `Token ${token}`,
...
/**
 * Get asset details from KoboToolbox
 */
export async function getKoboAsset(
  token: string,
  assetUid: string
): Promise<KoboAsset> {
  try {
...
/**
 * Fetch submissions from a deployed form
 */
...
/**
 * Generate a shareable link for a deployed form
 */
...
/**
 * Generate a public survey link
 */
export function generatePublicSurveyLink(assetUid: string, deploymentUid: string): string {
  return `https://kf.kobotoolbox.org/forms/${deploymentUid}`;
}

/**
 * Publish questionnaire to KoboToolbox
 */
export async function publishToKoboToolbox(
  ...
  try {
    ...
    // Create asset
    const asset = await createKoboAsset(
      token,
      config.projectName,
      xlsForm,
      {
    ...
    return {
      ...
      version: parseInt(questionnaire.version, 10) || 1,
...
/**
 * Sync responses from KoboToolbox back to FieldScore
 */
export async function syncResponses(
  token: string,
  assetUid: string,
  questionnaiireId: string,
  lastSyncTime?: string
): Promise<SyncResult> {
  try {
    const submissions = await getKoboSubmissions(token, assetUid);

    // Filter submissions based on last sync time if provided
    let filtered = submissions.submissions;
    if (lastSyncTime) {
      const lastSync = new Date(lastSyncTime);
      filtered = submissions.submissions.filter((s) => {
        const modified = new Date(s._date_modified);
