...
import { Check, Loader, AlertCircle, BarChart3, Database, GitBranch, Zap } from 'lucide-react';
...
interface PipelineStatus {
  ...
  quality_metrics: Record<string, { completion_rate?: number }>;
}
  ...
  const handlePublish = async () => {
    try {
      ...
      await httpClient.post(
        ...
        { auto_sync_enabled: true, sync_interval_minutes: 10 }
      );
      setActiveStep(1);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      {error && (
        <div style={{
          ...
          borderRadius: 8, color: '#DC2626', display: 'flex', alignItems: 'center', gap: 8, fontSize: 12
        }}>
          <AlertCircle size={16} />
          {error}
        </div>
      ...
      <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 12, padding: 24, position: 'relative' }}>
        ...
        <div style={{ position: 'relative' }}>
          <div style={{ position: 'absolute', top: 20, left: '5%', right: '5%', height: 2, background: '#E2E8F0', zIndex: 0 }} />
              ...
              return (
                <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  ...
                  <div style={{ textAlign: 'center', marginBottom: 8 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#080D1A', marginBottom: 2 }}>{step.title}</div>
                    <div style={{ fontSize: 10, color: '#9CA3AF', lineHeight: 1.3 }}>{step.description}</div>
                  </div>
                  ...
                  {step.action && (
                    <button
                      ...
                      style={{
                        ...
                        background: '#2463EB', color: 'white', border: 'none', borderRadius: 4,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16 }}>
        <StatCard icon={<Database size={18} />} title="Total Responses" value={status.total_responses} color="#2463EB" />
        <StatCard icon={<GitBranch size={18} />} title="Sources" value={status.response_sources.length} color="#7C3AED" />
        <StatCard
      ...
      {status.response_sources.length > 0 && (
        <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 12, padding: 16 }}>
          <h4 style={{ fontSize: 12, fontWeight: 700, marginBottom: 12, color: '#080D1A' }}>Response Sources</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {status.response_sources.map((source, idx) => (
              <div key={idx} style={{
                ...
                <div style={{
                  fontSize: 12, fontWeight: 700, color: '#2463EB',
...
function StatCard({ icon, title, value, color }: { icon: React.ReactNode; title: string; value: string | number; color: string }) {
  return (
    <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 12, padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ color, display: 'flex' }}>{icon}</div>
      <div style={{ fontSize: 10, color: '#9CA3AF', fontWeight: 600, textTransform: 'uppercase' }}>{title}</div>
      <div style={{ fontSize: 20, fontWeight: 700, color: '#080D1A' }}>{value}</div>
    </div>
  );
}
