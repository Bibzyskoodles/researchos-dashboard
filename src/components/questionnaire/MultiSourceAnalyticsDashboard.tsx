...
export default function MultiSourceAnalyticsDashboard({ questionnaireId }: MultiSourceAnalyticsDashboardProps) {
  ...
  const handleExport = async () => {
    try {
      const response = await httpClient.post(
        `/api/v2/analytics/questionnaires/${questionnaireId}/export-analytics`,
        { format: 'csv' }
      );
  ...
  if (loading) {
    return <div style={{ padding: 24, textAlign: 'center', color: '#6B7280' }}>Loading analytics...</div>;
  }

  if (error) {
    return (
      <div style={{ padding: 24, background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, color: '#DC2626', display: 'flex', alignItems: 'center', gap: 12 }}>
        <AlertCircle size={20} />
        <span>{error}</span>
      </div>
    );
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', border: '1px solid #E2E8F0', borderRadius: 6, background: 'white', cursor: 'pointer', fontSize: 12, fontWeight: 500, color: '#2463EB' }}
          >
            <RefreshCw size={14} /> Refresh
          </button>
          <button
            onClick={handleExport}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', background: '#2463EB', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontWeight: 500 }}
          >
      ...
      <div style={{ display: 'flex', gap: 16, borderBottom: '1px solid #E2E8F0' }}>
        {(['overview', 'quality', 'sync'] as const).map(tab => (
          <button
            ...
            style={{ padding: '12px 16px', fontSize: 14, fontWeight: 500, border: 'none', background: 'none', cursor: 'pointer', color: activeTab === tab ? '#2463EB' : '#6B7280', borderBottom: activeTab === tab ? '2px solid #2463EB' : 'none', textTransform: 'capitalize' }}
          >
            {tab}
          </button>
        ))}
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 24 }}>
      <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 8, padding: 16 }}>
        <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 16, color: '#080D1A' }}>Response Distribution</h3>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {data.sources.map((source, idx) => (
          <div key={idx} style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 8, padding: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: source.source === 'fieldscore_direct' ? '#2463EB' : '#7C3AED' }}>
              {source.percentage}%
            </div>
          </div>
        ))}
...
function QualityTab({ data }: { data: AnalyticsData }) {
  const metricKeys: Array<{ label: string; key: keyof QualityMetric }> = [
  ...
  const chartData = metricKeys.map(m => ({
    name: m.label,
    fieldscore_direct: data.quality_metrics.fieldscore_direct[m.key] as number,
    kobotools: data.quality_metrics.kobotools[m.key] as number,
  }));

  return (
    <div style={{ marginTop: 24 }}>
      <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 8, padding: 16 }}>
        <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 16, color: '#080D1A' }}>Quality Comparison</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginTop: 16 }}>
        {([
          ...
          <div key={section.title} style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 8, padding: 16 }}>
            <h4 style={{ fontSize: 13, fontWeight: 600, marginBottom: 12, color: '#080D1A' }}>{section.title}</h4>
            {(Object.entries(section.metrics) as Array<[string, string | number]>).map(([key, value]) =>
              key !== 'source' ? (
      ...
      <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 8, padding: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 13, fontWeight: 600, color: getStatusColor(syncStatus.sync_status), textTransform: 'capitalize' }}>
