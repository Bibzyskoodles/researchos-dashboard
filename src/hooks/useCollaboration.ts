...
export function useCollaboration(options: UseCollaborationOptions): UseCollaborationReturn {
  ...
  const shareQuestionnaire = useCallback(
    async (req: ShareQuestionnaireRequest) => {
      const result = await apiCall('POST', `/${questionnaireId}/share`, req);
  ...
  const updateAccess = useCallback(
    async (userId: string, role: string) => {
      await apiCall('PUT', `/${questionnaireId}/access/${userId}`, { role });
      setCollaborators((prev) =>
        prev.map((c) => (c.user_id === userId ? { ...c, role: role as Collaborator['role'] } : c))
      );
    },
    [questionnaireId, apiCall]
  );
  ...
  const addComment = useCallback(
    async (req: AddCommentRequest): Promise<Comment> => {
      const result = await apiCall('POST', `/${questionnaireId}/comments`, req);
  ...
  const updateComment = useCallback(
    async (commentId: string, content: string) => {
      await apiCall('PUT', `/${questionnaireId}/comments/${commentId}`, { content });
  ...
  const resolveComment = useCallback(
    async (commentId: string) => {
      await apiCall('POST', `/${questionnaireId}/comments/${commentId}/resolve`, {});
      setComments((prev) =>
  ...
  const getActivityFeed = useCallback(
    async (limit = 50) => {
      const result = await apiCall('GET', `/${questionnaireId}/activity?limit=${limit}`);
  ...
  const getVersions = useCallback(async () => {
    const result = await apiCall('GET', `/${questionnaireId}/versions`);
    setVersions(result.versions);
  }, [questionnaireId, apiCall]);

  const restoreVersion = useCallback(
    async (versionId: string) => {
      await apiCall('POST', `/${questionnaireId}/versions/${versionId}/restore`, {});
      await getVersions();
    },
  ...
  const updatePresence = useCallback(async () => {
    await apiCall('POST', `/${questionnaireId}/presence/update`, {
      position: { focused: true },
      color: userColorRef.current,
    });
    const result = await apiCall('GET', `/${questionnaireId}/presence`);
    setPresence(result.presence);
  }, [questionnaireId, apiCall]);

  const getNotifications = useCallback(
    async (unreadOnly = false) => {
      const result = await apiCall('GET', `/notifications?unread=${unreadOnly}`);
  ...
  const markNotificationRead = useCallback(
    async (notificationId: string) => {
      await apiCall('POST', `/notifications/${notificationId}/read`, {});
          ...
          try {
            ...
            switch (msg.type) {
              case 'user_joined':
                ...
                break;
              case 'user_left':
                setCollaborators((prev) => prev.filter((c) => c.user_id !== msg.data.user_id));
                break;
              case 'cursor_moved':
                setPresence((prev) =>
                  prev.map((p) =>
                    p.user_id === msg.data.user_id ? { ...p, cursor_position: msg.data.position } : p
                  )
                );
                break;
              default:
  ...
  return {
    collaborators, comments, activities, versions, presence, notifications,
    isLoading, error,
    shareQuestionnaire, updateAccess, addComment, updateComment, resolveComment,
    getActivityFeed, getVersions, restoreVersion, updatePresence,
    getNotifications, markNotificationRead, refreshAll,
  };
}
...
export function useCommentThread(questionnaireId: string, commentId: string, token: string) {
  ...
  const addReply = useCallback(
    async (content: string, mentions?: string[]) => {
      setIsLoading(true);
      try {
        const response = await fetch(`/api/questionnaires/${questionnaireId}/comments`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({ content, mentions, parent_comment_id: commentId }),
        });
  ...
  return { replies, isLoading, addReply };
}
...
export function useNotifications(
  ...
  const fetchNotifications = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/questionnaires/notifications', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const markAsRead = useCallback(
    async (notificationId: string) => {
      try {
        await fetch(`/api/questionnaires/notifications/${notificationId}/read`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
        });
