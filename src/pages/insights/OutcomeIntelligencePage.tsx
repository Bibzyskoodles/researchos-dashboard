...
export default function OutcomeIntelligencePage() {
  ...
  const handleFileUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = ev => {
      try {
        ...
        setError('Could not parse file. Ensure it is a valid JSON or CSV export from FieldScore.');
      }
    };
  ...
  const exportReport = useCallback(() => {
    ...
    const lines = [
      ...
      `FieldScore · Generated ${new Date().toLocaleString('en-GB')}`,
      '═'.repeat(56),
      '',
      `INFERRED OBJECTIVE`,
      ...
      '',
      '─'.repeat(56),
      'Generated by FieldScore Outcome Intelligence',
    ];
          ...
          <div style={{ background: 'white', borderRadius: 16, border: '1px solid #E8EDF5', overflow: 'hidden', boxShadow: '0 2px 12px rgba(10,15,28,.06)' }}>
            ...
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ fontSize: 10.5, color: '#CBD5E1', textAlign: 'center', lineHeight: 1.5 }}>
                Works with any dataset — KoboToolbox exports, ODK, or FieldScore CSVs
              </div>
            </div>
          </div>
