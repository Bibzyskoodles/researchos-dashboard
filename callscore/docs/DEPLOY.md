...
| `SPITCH_API_KEY` | Spitch key | **Nigerian-language specialist STT** (Yoruba/Igbo/Hausa/Nigerian English). Cross-check slot — a Deepgram-vs-specialist disagreement on code-switched audio is real signal. Set `SPITCH_LANGUAGE` (en/yo/ig/ha); verify the endpoint against docs.spitch.app on first use (`SPITCH_API_URL` configurable). |
