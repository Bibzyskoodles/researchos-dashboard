...
| `SPITCH_API_KEY` | Spitch key | **Nigerian-language specialist STT** (Yoruba/Igbo/Hausa/Nigerian English). Slots in as the cross-check voice ahead of AssemblyAI — a Deepgram-vs-Spitch disagreement on code-switched audio is real signal. Set `SPITCH_LANGUAGE` (en/yo/ig/ha) per deployment; verify the endpoint against docs.spitch.app on first use (`SPITCH_API_URL` is configurable). |
